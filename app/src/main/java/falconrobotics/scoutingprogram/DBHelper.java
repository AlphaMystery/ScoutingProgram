...
public class DBHelper
{
    ...
    public static final String DATABASE_NAME = "AZFL2016.db";
    ...
    private static void createDir() {
        try {
            ...
            if (!picDir.exists()) {
                if (!picDir.mkdirs()) {
                    //  WARN USER
                    Toast.makeText(MainActivity.context, "WARNING! UNABLE TO CREATE WORKING DIRECTORY.", Toast.LENGTH_LONG).show();
                }
            }
    ...
    public static void create() {
        createDir();
        SQLiteDatabase db = SQLiteDatabase.openDatabase(dbDirPath + File.separator + DATABASE_NAME, null,
    ...
    public synchronized void pit_InsertReplace(Model_Pit pitObject)
    {
        SQLiteDatabase db = SQLiteDatabase.openOrCreateDatabase(dbDirPath + File.separator + DATABASE_NAME, null, null);
        if (db != null) {
            db.execSQL("INSERT OR REPLACE INTO " + TABLE_PIT
                    + "(_id, yearDriver, wheels, weight, shooter, canClimb, climbSpeed"
                    + ", portcullis, chevalDeFrise, moat, ramparts, drawbridge, sallyPort, rockWall"
                    + ", roughTerrain, lowBar, comments, robotPhoto, syncNum)"
                    + " VALUES(" +
                    pitObject.get_id() + ", " + pitObject.getYearDriver() + ", " + pitObject.getWheels() + ", " + pitObject.getWeight() + ", " + pitObject.getShooter() + ", " + pitObject.getCanClimb() + ", " + pitObject.getClimbSpeed() + ", " +
            ...
            Toast.makeText(MainActivity.context, db.isOpen() + " < isOpened()", Toast.LENGTH_LONG).show();
        }
